...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="writeCssFile">
        <xsl:result-document href="{testng:absolutePath('style.css')}" format="text">
            ...
            body { font-family: Arial, sans-serif; font-size: 12px; padding: 10px; margin: 0px; background:white url('images/pear-icon_purple.png') no-repeat scroll top right; }
            ...
            .exceptionDetails, .exceptionDetailsVisible { padding: 5px; background-color: #FF947F; margin: 1px; }
			...
			.individualRunsPass td { background-color: #F5F5F5}
...
.infinity { background-color: rgb(200,120,120) !important;}
    ...
    <xsl:template name="writeJsFile">
        <xsl:result-document href="{testng:absolutePath('main.js')}" format="text">
			...
			function formatLog(docEleid) {
				...
				var new_str1 = new_str.replace(/                                             /gi, "");
				document.getElementById(docEleid).innerHTML = new_str1;
            }

            function formatTestLog(docEleid) {
                var str = document.getElementById(docEleid).innerHTML;
				var new_str = str.replace(/                                                         /gi, "");
    ...
    <xsl:template name="htmlHead">
        <head>
            <title>
                <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'API Test Results')"/>
            </title>
                                        ...
                                        <td colspan="6">
                                            <!--<pre>
                                                <xsl:variable name="TElogMessage"
                                                              select="unparsed-text(concat('file:///', system-property('user.dir'), '/lapitaf.log'))"/>
                                                <xsl:value-of select="$TElogMessage"/>
                                            </pre>-->
                                            <!--<pre>
                                                <xsl:choose>
                                                    <xsl:when
                                                            test="unparsed-text-available(concat('file:///', system-property('user.dir'), '/target/log/lapitaf.log'))">
                                                        <xsl:variable name="TElogMessage"
                                                                      select="unparsed-text(concat('file:///', system-property('user.dir'), '/target/log/lapitaf.log'))"/>
                                                        <xsl:value-of select="$TElogMessage"/>
                                                    </xsl:when>
                                                    <xsl:otherwise>
                                                        <xsl:variable name="TElogMessage"
...
<!--    <xsl:template name="testStepsTemplate">
        ...
        <xsl:variable name="xmlRaw"
                      select="$xmlData/testng-results/suite[@name=$inSuiteName]/test[@name=$inTestName]/class[@name=$inClassName]/test-method[@started-at=$in-started-at and @finished-at=$in-finished-at and @name=$in-name and @status=$in-status]/logs/log"/>
   ...
   <!-- <xsl:template name="testLogsTemplate">
        <xsl:param name="inClassName"/>
        <xsl:param name="inStepName"/>
        <xsl:variable name="TSlogMessage"
                      select="unparsed-text(concat('file:///', system-property('user.dir'), '/log/', $inClassName, '/', 'testLog.log'))"/>
        <tr>
            <td>
                <xsl:value-of select="$TSlogMessage"/>
            </td>
        </tr>

    </xsl:template>-->

    <!--<xsl:template name="testLogTemplate">
        <xsl:param name="inClassName"/>

        &lt;!&ndash;<a onclick="toggleLogsVisibility('{concat(../@name, 'methodLog')}')">
            <xsl:value-of select="'Log'"/>
        </a>&ndash;&gt;
        <div id="{concat(../@name, 'methodLog')}" class="logDetails">
            <div>
                <table width='75%'>
                    <tbody>
                        <tr>
                ...
                <table width='75%'>
                    <tbody>
                        <tr nowrap="true">
                            <td colspan="6">
                                <pre>
                                    &lt;!&ndash;<xsl:variable name="TElogMessage"
                                                  select="unparsed-text(concat('file:///', system-property('user.dir'), '/../', 'lapitaf.log'))"/>
                                    <xsl:value-of select="$TElogMessage"/>&ndash;&gt;
                                    <xsl:variable name="TSlogMessage" select="unparsed-text(concat('file:///', system-property('user.dir'), '/log/', $inClassName, '/', 'testLog.log'))"/>
                                    <xsl:value-of select="$TSlogMessage"/>
                                    &lt;!&ndash;<xsl:if test="not(@is-config)">
                                        <xsl:call-template name="testLogTemplate">
                                            <xsl:with-param name="inClassName" select="../@name"/>
                                        </xsl:call-template>
                                    </xsl:if>&ndash;&gt;
                                </pre>
                            </td>
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr>
                <td colspan="6" class="detailsBox">
                    <div id="{$detailsId}" class="testMethodDetails" style="overflow-x: scroll; width: 1000px;">
                        <!--<table>
                            <tr>
                            ...
                            <div class="section">
                                <table class="testMethodsTable individualRunsPass individualRuns" cellpadding="0"
                                    ...
                                    <!-- <xsl:variable name="rptGraphPath"
                                                   select="concat('file:///', system-property('user.dir'), '/target/surefire-reports/Test_Steps-processed.xml')"/>
                                    ...
                                    <!--<xsl:if test="not(@is-config)">
                                        <xsl:call-template name="testLogsTemplate">
                                            <xsl:with-param name="inClassName" select="../@name"/>
                                        </xsl:call-template>
                                    </xsl:if>-->
                                ...
                                <table class="testMethodsTable individualRunsPass individualRuns" cellpadding="0"
                                    ...
                                    <xsl:if test="not(@is-config)">
                                        ...
                                        <div id="log_134" class="logDetails">
                                            <!--<table width='75%'>
                                                <tbody-->>
                                            <tr nowrap="true">
                                                <td colspan="6">
                                                    <pre>
                                                        ...
                                                        <xsl:value-of select="$TElogMessage"/>-->
                                                        <xsl:variable name="TSlogMessage"
                                                                      select="unparsed-text(concat('file:///', system-property('user.dir'), '/target/log/', ../@name, '/', 'testLog.log'))"/>
                                                        <xsl:value-of select="$TSlogMessage"/>
                                                        <!--<xsl:if test="not(@is-config)">
                                                            <xsl:call-template name="testLogTemplate">
                                                                <xsl:with-param name="inClassName" select="../@name"/>
                                                ...
                                                <td colspan="6">
                                                    <xsl:choose>
                                                        <xsl:when test="exception/full-stacktrace">
                                                            <pre style="padding: 0px; margin-left:0; margin-bottom:-40;">
                                                                <xsl:value-of
    ...
    <xsl:template name="powered-by">
        <div style="margin-top: 15px; color: gray; text-align: center; font-size: 9px;">
            Copyright ©
            <a href="http://www.Leapset.com/" style="color: #8888aa;" target="_blank">
                Leapset QA
            </a>
        </div>
    </xsl:template>
